...
for firm in firms:
    ...
    for tit, info in zip(soup_tit, soup_info):
        if firm in tit.get_text():
            ...
            news_data.append([info.get_text(),tit.get_text(),tit['href']])
...
#테이블 생성 명령어
cursor.execute("CREATE TABLE NEWSDATA(WRTN_TM TIMESTAMP, TITLE VARCHAR(100) NOT NULL, URL VARCHAR(200) NOT NULL)")
...
for firm in firms:
    ...
    for tit, info in zip(soup_tit, soup_info):
        if firm in tit.get_text():
            ...
            query="INSERT INTO NEWSDATA VALUES (\'"+wrtn_tm+"\',\'"+title+"\',\'"+tit['href']+"\')"
