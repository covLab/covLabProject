...
//		System.out.println(vds);
	...
	/*
	 * public String getData() { reservationService rservice = new
	 * reservationService(); StringBuffer result=new StringBuffer();
	 * ArrayList<Hospital> hps = rservice.selectAllHps(); ArrayList <VaccineData>
	 * vds=rservice.selectAllVds();
	 * 
	 * for (Hospital hp : hps) { for (VaccineData vd : vds) { hp.setRemain(0);//병원에
	 * 잔여 백신데이터가 없으면 remain 0 if (hp.getReg_bus_no()==vd.getReg_bus_no()) {
